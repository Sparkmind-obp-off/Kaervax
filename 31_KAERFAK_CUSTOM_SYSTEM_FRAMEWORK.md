...
## 2. Delivery
Discover → scope → architecture → proposal → approval/payment → build → test → handover → support.

## 3. Private Instance
A private instance may use the same underlying capabilities as a public product while keeping its data, configuration, and identity isolated.

## 4. Portfolio Treatment
A private system may become a portfolio example only when:
- privacy is preserved,
- sensitive information is removed,
- permission is available when required,
